...
| **aside-browser** | Skill + CLI | cskwork/promptbox (skills/aside-browser) | Hands browser work to the Aside browser, which drives sites you are already signed into — internal dashboards and admin pages with no usable API — via MCP or `aside "..."`. **macOS only, and installed by default there**; other platforms skip it and use Playwright. The skill and CLI install automatically, but stay inert until you install and sign into the app |
