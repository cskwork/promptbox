...
[`cskwork/pi-setup`](https://github.com/cskwork/pi-setup) repository into `~/pi-setup`, and runs its
installer to restore the complete environment.

## What it does

1. Checks Node.js 22+ and `pi`, installing only what is missing.
2. Synchronizes `~/pi-setup` with `git pull --ff-only` without discarding local work.
