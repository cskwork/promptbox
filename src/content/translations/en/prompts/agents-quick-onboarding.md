...
| **eli5** (set-independent, always installed) | Skill | [anthropics/claude-plugins-community](https://github.com/anthropics/claude-plugins-community) (`eli5/skills/eli5`) | Turns any topic into a picture book: one `/eli5 <topic>` keystroke renders a standalone HTML artifact that explains with big pictures and few words. No pipeline, no router — it cannot conflict with a workflow set |
...
- **Skill routing** — `~/.agents/skills/` is the hub. The <span class="tip" tabindex="0" data-tip="The router is the entry skill that decides which skill handles a task. Set A: using-agent-skills, B: using-superpowers, C: ask-matt.">router</span> of the set you chose in 4a is installed too, but the instruction file **never forces a skill**: every mention stays conditional ("if intent is unclear, run X"), never "always route through X" — a mandatory router would make the agent load a skill before it may answer even a one-line question.
