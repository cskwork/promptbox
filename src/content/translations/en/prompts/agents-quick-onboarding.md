...
| **aside-browser** | Skill + CLI | cskwork/promptbox (skills/aside-browser) | Hands browser automation — QA, element interaction, snapshots, screenshots — and any work spanning your logged-in accounts, apps (Slack, X, LinkedIn), and browsing history to Aside. `aside exec` delegates a whole task to Aside's own agent; `aside repl` is a Playwright-compatible session for evidence and deterministic steps. **macOS only, and installed by default there**; other platforms skip it and use Playwright. The skill and CLI install automatically, but stay inert until you install and sign into the app |
...
- **The browser app needs a human to finish installation (aside-browser).** On macOS the prompt installs the skill and the `aside` CLI by default, but the [Aside](https://aside.com/) app is a DMG whose first run is a GUI step asking privacy-sensitive questions — import your Chrome data? unlock a credential vault? — that an agent must never answer for you. So the state right after setup is "skill present, CLI present, no account," and that is `PENDING-USER`, not a failure. Aside is **macOS-only** today; elsewhere it is recorded as `SKIPPED-UNSUPPORTED` and **Playwright** is the route. Playwright also stays the right choice on macOS for CI, headless runs, and deterministic regression — an Aside run is evidence of a one-time outcome, not a reproducible gate.
