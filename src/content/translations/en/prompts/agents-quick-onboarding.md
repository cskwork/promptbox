...
- **This prompt installs no browser at all.** A desktop browser arrives as a DMG whose first run is a GUI step asking privacy-sensitive questions — import your Chrome data? unlock a credential vault? — that an agent must never answer on your behalf, so browser installation is out of scope entirely. Agent-driven web automation goes through **Playwright** on every platform. For a day-to-day browser the prompt will point you at [Aside](https://aside.com/) if you ask, with the caveat that it is **macOS-only** today and is a standalone app with its own built-in agent, not something a coding CLI can drive.
