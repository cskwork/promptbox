...
license: MIT
order: 25
trigger: "이거 왜 이렇게 만들었지 / 왜 Y 대신 X를 골랐나 / 이 상수는 어디서 나온 숫자인가 / 설계 근거·회귀·포스트모템 조사. 런타임 동작을 묻는 how 스킬과 짝이다."
install: "/add-plugin pstack (Cursor). references/ 하위 파일이 필요하므로 SKILL.md만 복사하면 안 되고 스킬 디렉터리 전체를 가져와야 한다."
---

## 한 줄

"이 코드는 왜 이렇게 생겼나"에 답하려고 git 이력만 뒤지는 대신, 사용 가능한 MCP를 실행 시점에
훑어 7개 증거 범주에 각각 서브에이전트 하나씩 붙이고 동시에 조사시킨 뒤, 인용이 달린 답을
합성하는 스킬이다. 런타임 동작을 설명하는 `how` 스킬의 짝이다.

## 언제 쓰는가

- 낯선 코드를 고치기 전에 원래 의도와 제약을 알아야 할 때. 마지막에 Preserve / Change / Avoid / Risk
  제약 목록으로 변환해 주므로 변경 계획의 입력이 된다
- `limit = 128 * 1024` 같은 매직 넘버가 어디서 나왔는지 추적할 때
- 회귀나 장애 후 "이 방어 코드는 어떤 사고 때문에 생겼나"를 되짚을 때
- 원 작성자가 퇴사해 물어볼 사람이 없을 때

## 무엇을 하는가

1. **코드 앵커 고정.** `git blame -L`, `git log --follow -p`, `gh pr view`로 파일·심볼·커밋·PR 번호를
   먼저 확보한다. 이걸 조사관들에게 넘겨 같은 작업을 반복하지 않게 한다.
2. **7개 조사관 병렬 실행.** 소스 관리, 이슈 트래커, 장문 문서, 팀 채팅, 인프라 관측, 에러 추적,
   제품 분석 웨어하우스. 각 범주에 MCP 하나씩 매핑하고, 한 에이전트에 여러 MCP를 몰아주지 않는다.
...
- **MCP가 없으면 반쪽이다.** 7개 중 6개가 외부 MCP(Linear, Notion, Slack, Datadog, Sentry, Databricks
  등)에 의존한다. git과 `gh`만 있는 환경에서는 소스 관리 조사관 하나만 돌고, 나머지는 "MCP 없음"
  갭으로 기록된다. 답이 틀리지는 않지만 얇아진다.
- **readonly 모드로 돌리면 조용히 망가진다.** SKILL.md가 명시한다. readonly는 MCP 접근을 벗겨내서
  MCP 기반 조사관을 전부 무력화한다. 서브에이전트는 agent 모드로 띄우되 쓰기를 안 하는 것은 규범으로
  지킨다.
- **하네스 종속.** `subagent_type: generalPurpose`, 모델 별칭(`grok-4.6-fast-xhigh`,
  `claude-fable-5-thinking-max`), `mcps/` 디렉터리 탐색이 Cursor 기준이다. 다른 하네스로 옮기면
  이 부분을 자기 환경에 맞게 바꿔야 한다.
- **참조 파일 5종이 별도로 있다.** `references/epistemics.md`, `investigator-prompt.md`,
  `source-playbook.md`, `sources/*.md`, `synthesizer-prompt.md`. 아래 SKILL.md만 복사하면 합성기가
  따라야 할 확신 등급표가 빠진다.
- **비용이 크다.** 조사관 최대 7개에 합성기 1개. 사소한 질문에 기본값대로 돌리면 과하다. 스킬도
  단일 커밋짜리 자명한 대상에는 인라인 답변을 허용하되 "드물어야 한다"고 못 박는다.
