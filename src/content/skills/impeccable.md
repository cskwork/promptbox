---
title: Impeccable
summary: "UI 설계·검토·다듬기, 접근성, 반응형 화면과 성능 개선을 돕는 디자인 스킬. pi 온보딩에 포함된 버전을 원문 그대로 제공합니다."
summary_en: "A design skill for building, reviewing, and refining interfaces. Includes the exact skill shipped with the default pi setup."
tags: [skill, design, frontend, accessibility, performance]
source: https://github.com/cskwork/pi-setup-public/tree/main/skills/impeccable
license: Apache-2.0
trigger: "/impeccable · UI 설계 · 디자인 검토 · 화면 다듬기 · 접근성 · 반응형 · 성능 개선"
---

## 언제 쓰는가

웹사이트나 앱의 UI(사용자 화면)를 설계하거나, 기존 화면의 가독성·배치·색상·접근성·성능을 개선할 때 사용합니다.
`/impeccable polish`는 화면 다듬기, `/impeccable audit`는 품질 점검에 사용합니다.

## 설치와 주의점

Promptbox의 [pi 온보딩 프롬프트](../../prompts/agents-quick-onboarding/)를 실행하면
`pi-setup-public`에 포함된 Impeccable이 `~/.pi/agent/skills/impeccable/`에 연결됩니다.
설치 프롬프트는 `SKILL.md`와 `reference/`, `scripts/` 폴더가 있는지 확인합니다.

이 스킬은 여러 파일로 구성됩니다. **SKILL.md만 복사해서는 참조 문서와 실행 스크립트가 설치되지 않습니다.**
아래 원문은 온보딩에 동봉된 버전이며, 독립 설치 안내는 [원본 프로젝트](https://github.com/pbakaus/impeccable)에서 확인할 수 있습니다.
