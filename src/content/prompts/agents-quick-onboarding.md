...
자주 쓰는 스킬과 공통 시스템 프롬프트를 `~/.agents/` **한 폴더**에 모으고, 설치된 모든 코딩
CLI가 <span class="tip" tabindex="0" data-tip="symlink — 한 파일을 여러 위치에서 가리키게 하는 바로가기. 원본 하나만 고치면 연결된 모든 도구에 즉시 반영된다.">심링크</span>로
그 폴더를 바라보게 만드는 설치 프롬프트 하나.
<span class="tip" tabindex="0" data-tip="idempotent — 몇 번을 다시 실행해도 결과가 같다. 정상인 항목은 건너뛰고, 빠졌거나 깨진 것만 고친다.">멱등</span>이라
몇 번을 돌려도 안전하고, 두 번째 실행부터는 복구 도구가 된다.

> 점선 밑줄이 있는 용어에 마우스를 올리면(모바일은 탭) 짧은 설명이 뜹니다.
...
## 프롬프트 내부 지도 — 붙여넣으면 무슨 일이 벌어지나

아래 코드블록은 길지만 구조는 단순하다. 번호 붙은 구간이 순서대로 실행되고, 각 구간은
"확인 → 기록 → 변경 → 재검증"의 리듬을 반복한다.

| 구간 | 하는 일 |
|---|---|
| 0 · 기본 규칙 | 모든 단계에 걸리는 안전 규칙. 핵심 셋 — "설치됨"의 기준은 `SKILL.md`가 실제로 읽히는가(폴더 존재는 증거가 아니다), 설치하려고 지우지 않는다(백업 후 이동만), 인터프리터는 절대경로로 고정 |
| 0b · 플랫폼 분기 | macOS/Windows를 한 번 확정하고 이후 모든 명령은 그 분기만 탄다. Linux는 macOS 분기에 치환 3개, WSL은 별도 설치로 취급 |
| 1–2 · 감지·인벤토리 | 설치된 CLI를 찾고, 손대기 전 상태를 `inventory.tsv`로 기록 — 이후 모든 복구의 근거 |
| 3 · 허브 생성 | `~/.agents/`를 단일 출처로 만든다 — 규칙·스킬 링크·소스 클론이 전부 여기 산다 |
| 4a · 세트 선택 | 비교표를 보여주고 A/B/C를 묻는다. **하나만** 설치하고, 바꿀 때는 쌓지 않고 교체한다 |
| 4 · 스킬 설치 | 레포를 클론하고 스킬별로 심링크. `git pull` 뒤에는 링크 너머 `SKILL.md`를 다시 확인한다 |
| 5·5b · 도구·브라우저 | officecli·herdr·rtk 설치. ego lite는 macOS 전용이고 첫 실행 GUI 온보딩은 사람이 끝낸다 |
| 5c · ai-memory | 공유 메모리 서버 1대를 네이티브로 깔고, 로그인 시 자동 시작을 등록하고, 감지된 모든 에이전트를 연결한다 |
| 5d · omp | 이미 있으면 연결만 하고, 없으면 설치하지 않는다 |
| 6·6b · MCP | MCP 서버를 설치하고 프로세스 기동이 아니라 실제 프로토콜 응답으로 검증한다. 코드 인덱서 MCP는 목록만 보여주고 제거는 사용자 승인 후 |
| 7·7b · 지시문 | 아래 "핵심 조항"의 시스템 프롬프트를 정본 한 파일로 쓰고 각 CLI의 경로를 심링크. `rules.md`는 절대 덮어쓰지 않는다 |
| 8–9 · 링크·검증 | 링크 상태별로 UNCHANGED/REPAIRED/REPLACED를 기록하고, **빈 폴더 0 · 깨진 링크 0**을 통과 조건으로 삼는다 |
| 10–11 · 복구·보고 | 읽기 전용 감사 스크립트를 남기고, 무엇이 바뀌었는지 보고한다 |

### 심어지는 시스템 프롬프트(AGENTS.md) — 핵심 조항

7단계가 모든 CLI에 심는 공통 시스템 프롬프트의 뼈대다. 코드블록 안
`# Operating Instructions`부터가 그 본문이다.

- **Stance(기본 태도)** — 도메인 데이터를 먼저 이해하고, 가장 작은 검증된 변경만 하고,
  검증하지 못한 것은 했다고 말하지 않는다.
- **규칙 분리** — 워크플로는 이 파일에, 환경마다 다른 도메인·안전 규칙은
  `~/.agents/rules/rules.md`에 둔다. 그래서 지시문 파일은 모든 머신에서 동일하게 유지된다.
- **Skill routing** — `~/.agents/skills/`가 허브다. 4a에서 고른 세트의
  <span class="tip" tabindex="0" data-tip="router — 작업을 보고 어떤 스킬을 부를지 정하는 진입 스킬. A는 using-agent-skills, B는 using-superpowers, C는 ask-matt.">라우터</span>도
  설치되지만, 지시문 파일은 **어떤 스킬도 강제하지 않는다** — 스킬 언급은 "의도가 불분명하면
  X를 돌려라"처럼 조건부로만 쓴다. "항상 X를 거쳐라"를 박아두면 한 줄짜리 질문에도 에이전트가
  스킬부터 로드해야 하기 때문이다.
- **8단계 실행 루프** — Orient(파악) → Options(**서로 다른 3안**을 제시하고 사용자가 고른다) →
  Delegate(<span class="tip" tabindex="0" data-tip="subagent — 본체와 별도의 깨끗한 컨텍스트로 도는 보조 에이전트. 큰 결과는 파일로 주고받는다.">서브에이전트</span> 위임) →
  Plan → Adversarial review(<span class="tip" tabindex="0" data-tip="계획에 스스로 구체적인 반론을 내고, 반론을 반영해 고치거나 계획이 살아남는 이유를 적어야 통과하는 게이트.">반론 게이트</span>) →
  Execute → Verify(실제 명령과 출력을 보여준다) → Report(정해진 보고 형식).
- **가볍게 유지** — 이 파일은 모든 에이전트가 매 세션 로드하는 가장 비싼 텍스트다.
  다이어그램이나 장문 설명은 넣지 않는다.

## 함정

전부 실제 실행에서 터진 것들이고, 프롬프트에 각각 방어 조항이 들어 있다.
가장 자주 터지는 다섯 가지만 추리면:

- **폴더가 있어도 설치된 게 아니다** — 판정은 <span class="tip" tabindex="0" data-tip="중간에 죽은 설치기는 이름만 맞는 빈 폴더를 남긴다. 실제로 심링크 43개가 이렇게 날아갔다. [ -d ... ] 검사는 금지.">`SKILL.md`가 읽히는가</span>로만 한다.
- **`git pull` 한 번에 스킬이 조용히 사라질 수 있다** — <span class="tip" tabindex="0" data-tip="상류 레포가 SKILL.md 위치를 옮기면 링크도 폴더도 멀쩡한데 스킬만 증발한다. 실제로 한 번의 갱신에 3개가 죽었다.">pull 뒤에는 링크 너머 `SKILL.md`를 다시 확인</span>한다.
- **설치·검증 스크립트를 그대로 믿지 않는다** — <span class="tip" tabindex="0" data-tip="상류 install.sh가 rm -rf를 하거나, 구현 안 된 --dry-run을 무시하고 실제 실행해버리는 사례가 있었다.">실행 전에 읽고</span>, 감사는 읽기 전용 스크립트로 따로 한다.
- **Windows는 시작 전에 세 가지부터** — <span class="tip" tabindex="0" data-tip="심링크 권한이 없으면 허브 모델 자체가 깨지고, longpaths가 없으면 클론이 죽고, autocrlf=true면 훅 스크립트가 bad interpreter로 죽는다.">개발자 모드(심링크 권한) · `core.longpaths` · `core.autocrlf`</span>.
- **ai-memory는 서버가 떠 있어야 기억한다** — <span class="tip" tabindex="0" data-tip="서버가 없어도 훅은 조용히 실패만 하고 에이전트는 공유 기억 없이 계속 일한다. 자동 시작 등록 후 포트 LISTEN을 눈으로 확인해야 한다.">자동 시작을 등록하고 LISTEN을 확인</span>한다.

<details>
<summary><strong>전체 함정 목록 (33개)</strong> — 증상 · 원인 · 방어책 전문</summary>
